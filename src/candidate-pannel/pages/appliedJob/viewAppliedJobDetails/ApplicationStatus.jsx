    ...
    if (!token) {
      ...
      try {
        ...
        if (response?.data?.jobs?.Shortlisted.length == 0) {
          ...
          response?.data?.jobs?.Shortlisted[0]?.candidate_id == userId &&
          response?.data?.jobs?.Shortlisted[0]?.shorted_status == false
        ) {
          setCurrentStep(2);
        } else if (
          response?.data?.jobs?.Shortlisted[0]?.shorted_status == true &&
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <button
            ...
            onClick={() => handleDownload(applicationState?.offerletterUrl)}
          >
            download
          </button>
        </div>
      ...
      <div className="stepper-container">
        <Row className="mb-4">
                ...
                <div
                  ...
                  style={{
                    ...
                    color:
                      index < currentStep - 1
                        ? "#3B96E1"
                        : currentStep === steps && index === steps - 1
                        ? "green"
                        : "#3B96E1",
         ...
         {currentStep==1?
          <div style={{ position: "absolute", left: "33%", fontSize: "10px" }}>
          {applicationState?.jobs?.applied_candidates[0]?.interviewRound
            .length > 0 &&
            (() => {
              const rounds =
                applicationState?.jobs?.applied_candidates[0]?.interviewRound;
              ...
              for (let i = rounds.length - 1; i >= 0; i--) {
                if (rounds[i].roundAction === "Selected") {
                  roundToShow = (
                    <span key={rounds[i]._id} style={{ color: "green" }}>
                  ...
                  roundToShow = (
                    <span key={rounds[i]._id} style={{ color: "red" }}>
          ...
          {!applicationStatus && currentStep == 2 && (
            <div className="company-ratings">
              ...
              <div className="view-company-ratings mt-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  <span
                    ...
                    style={{
                      cursor: "pointer",
                      color: star <= rating ? "#ffc107" : "#BFBFBF",
                      fontSize: "2rem",
                      marginTop: "-14px",
                    }}
              ...
              <textarea
                ...
                style={{ color: "black" }}
              />
              <Button onClick={AddFeedaBack} style={{ width: "100%" }}>
                Submit
              </Button>
            </div>
          )}
          {currentStep == 3 && (
            <div className="main-view-offered">
              ...
              >
                Offer received on{" "}
                {formatDate(
                  applicationState?.jobs?.Shortlisted[0]?.sortlisted_date
                )}
              </p>
            </div>
          )}
